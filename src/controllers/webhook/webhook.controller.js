...
const AssignedDisputeToStaff = async ({
  ids,
  merchantId,
  disputeId,
  staffState,
  ...
  try {
    ...
    if (_.isEmpty(merchantId)) {
      throw new AppError(
        ...
        AppErrorCode.fieldIsRequired("merchantId")
      );
    }
    ...
    if (merchantId?.length !== 15 || merchantId.slice(0, 3) !== "MID") {
      throw new AppError(
        statusCodes.BAD_REQUEST,
        AppErrorCode.InvalidFieldFormat("MerchantId")
      );
    }
    ...
    if (_.isEmpty(parsePayload)) {
      throw new AppError(
        statusCodes.BAD_REQUEST,
        "Unsupported Gateway Payload :" + Gateway
      );
    }

    // Step 6  : Normalize the Gateway Payload  ==> Adaptor Layer  git status
    ...
    if (await schemaValidator(normalizePayloadSchema, normalizePayload, res)) {
      throw new AppError(
        statusCodes.BAD_REQUEST,
        "normalize payload validation failed"
      );
    }
    ...
    let [payloadData, dispute, customId, staffMembers, staffAssignmentState] =
      await Promise.all([
        ...
        Dispute.findOne({
          where: {
            disputeId: normalizePayload?.disputeId,
            merchantId: merchant.id,
          },
          attributes: [
            "id",
    ...
    const isExist = !!dispute; //    !null ?=> !true => false
    ...
    if (isExist) {
      ...
      if (_.isEmpty(historyRecord)) {
        ...
        throw new AppError(
          statusCodes.BAD_REQUEST,
          "Failed to Update Dispute History Status"
        );
      }
      ...
      if (!_.isEmpty(staffMembers) && !dispute?.staffId) {
        ...
        if (ids.length > 0) {
          ...
          const nextStaff = staffMembers.find(
          ...
          const { title, message } = generateDisputeNotificationTemplate(
            customId,
          ...
          const { title: title2, message: message2 } =
            generateDisputeNotificationTemplate(
              dispute.customId,
              DisputeStatus.EVENT_CHANGED_ASSIGNED_STAFF,
              staffName,
              { newStatus: dispute.event }
            );
          notify.push({
        ...
        if (dispute?.staffId) {
          ...
          const { title, message } = generateDisputeNotificationTemplate(
            dispute.customId,
            DisputeStatus.DISPUTE_RECEIVED_UNASSIGNED,
            "",
            {}
          );
          notify.push({
      ...
      if (!_.isEmpty(staffMembers)) {
        ...
        if (ids.length > 0) {
          ...
          const { title: title2, message: message2 } =
            generateDisputeNotificationTemplate(
              dispute.customId,
              DisputeStatus.DISPUTE_RECEIVED_MERCHANT,
              staffName,
              {}
            );
          notify.push({
    ...
    const logMessage = isExist
      ? `${dispute.customId} status Updated`
      : `New Dispute Added with id -> ${dispute.customId}`;
    ...
    return res
      ...
      .json({
        message: error?.message || "Failed to receive Dispute From Gateway",
      });
  }
};
